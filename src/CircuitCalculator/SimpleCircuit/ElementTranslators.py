...
SchemdrawElementTranslator = Callable[[SchemdrawElement, Callable[[util.Point], str]], tuple[Branch, str]]
...
def linear_current_source_translator(element: elm.RealCurrentSource, node_mapper: Callable[[util.Point], str]) -> tuple[Branch, str]:
...
def resistor_translator(element: elm.Resistor, node_mapper: Callable[[util.Point], str]) -> tuple[Branch, str]:
    n1, n2 = get_nodes(element)
    return Branch(node_mapper(n1), node_mapper(n2), network_elmements.resistor(element.R)), element.name

def current_source_translator(element: elm.CurrentSource, node_mapper: Callable[[util.Point], str]) -> tuple[Branch, str]:
    n1, n2 = get_nodes(element)
    return Branch(node_mapper(n1), node_mapper(n2), network_elmements.current_source(element.I)), element.name

def linear_voltage_source_translator(element: elm.RealVoltageSource, node_mapper: Callable[[util.Point], str]) -> tuple[Branch, str]:
    n1, n2 = get_nodes(element)
    return Branch(node_mapper(n1), node_mapper(n2), network_elmements.linear_voltage_source(element.V, element.R)), element.name

def voltage_source_translator(element: elm.VoltageSource, node_mapper: Callable[[util.Point], str]) -> tuple[Branch, str]:
    n1, n2 = get_nodes(element)
    return Branch(node_mapper(n2), node_mapper(n1), network_elmements.voltage_source(element.V)), element.name

def new_linear_current_source_translator(element: elm.RealCurrentSource, node_mapper: Callable[[util.Point], str]) -> tuple[Branch, str]:
    n1, n2 = get_nodes(element)
