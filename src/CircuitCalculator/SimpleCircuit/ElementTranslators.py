...
def real_voltage_source_translator(element: elm.RealVoltageSource, node_mapper: Callable[[schemdraw.util.Point], str]) -> tuple[ntw.Branch, str]:
    n1, n2 = get_nodes(element)
    return ntw.Branch(node_mapper(n1), node_mapper(n2), ntw.real_voltage_source(element.V, element.R)), element.name
...
def new_real_current_source_translator(element: elm.RealCurrentSource, node_mapper: Callable[[schemdraw.util.Point], str]) -> tuple[ntw.Branch, str]:
    n1, n2 = get_nodes(element)
