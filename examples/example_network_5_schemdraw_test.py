...
with sd.Drawing() as d:
    d += sdw.VoltageSource(V=Uq1, name='Uq1', reverse=True).up()
    d += sdw.Line().up()
    d += (_R1:=sdw.Resistor(R=R1, name='R1').right())
    d += sdw.VoltageSource(V=Uq2, name='Uq2', reverse=True).down()
    d += sdw.Resistor(R=R3, name='R3').down()
    d += (_R2:=sdw.Resistor(R=R2, name='R2').right().at(_R1.end))
    d += sdw.VoltageSource(V=Uq3, name='Uq3', reverse=True).down()
    d += sdw.Resistor(R=R4, name='R4').down()
    d += sdw.Line().right().at(_R2.end)
    d += sdw.Line().down()
    d += sdw.CurrentSource(I=Iq4, name='Iq4', reverse=True).down()
    d += sdw.Line().left()
    d += sdw.Line().left()
    d += sdw.Line().left()
    ...
    d += schemdraw_solution.draw_voltage('Uq1', reverse=True)
