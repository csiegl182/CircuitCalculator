...
from CircuitCalculator.dump_load import restore_complex
import numpy as np
from numpy.testing import assert_approx_equal
import pytest

@given(st.complex_numbers(allow_nan=False))
def test_restore_complex_from_real_imag_data(z: complex) -> None:
    data = {'z': {'real': z.real, 'imag': z.imag}}
    restored_data = restore_complex(data)
...
def test_restore_complex_from_abs_phase_data(z: complex) -> None:
...
def test_restore_complex_from_abs_phase_data_with_negative_abs_raises_value_error() -> None:
    data = {'z': {'abs': -1, 'phase': np.pi}}
    with pytest.raises(ValueError):
        _ = restore_complex(data)

@given(st.complex_numbers(allow_nan=False, allow_infinity=False))
def test_restore_complex_from_abs_phase_data_with_deg_phase(z: complex) -> None:
