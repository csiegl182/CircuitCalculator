...
def test_admittance_translator_accepts_complex_y_value() -> None:
    admittance_dict = {
        'type': 'admittance',
        'id': 'Y1',
        'nodes': ('0', '1'),
    ...
    assert admittance.value['G'] == complex(admittance_dict['value']['Y'].replace(' ', '')).real
    assert admittance.value['B'] == complex(admittance_dict['value']['Y'].replace(' ', '')).imag
