import pytest
from CircuitCalculator.Network.network import Network, Branch, FloatingBranch
...
def test_network_returns_floating_branch_error_if_branch_not_connected_to_reference_node() -> None:
    R1, R2, R3, R4 = 10, 20, 30, 40
    branchA = Branch('0', '1', resistor('R1', R1))
    branchB = Branch('1', '2', resistor('R2', R2))
    branchC = Branch('2', '0', resistor('R3', R3))
    branchD = Branch('a', 'b', resistor('R4', R4))
    network = Network([branchA, branchB, branchC, branchD], reference_node_label='0')
    with pytest.raises(FloatingBranch):
        network['R4']
