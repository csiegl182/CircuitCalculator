...
def test_voltage_sources_are_substituted_by_open_circuit() -> None:
    network = Network([Branch('n1', 'n2', elm.voltage_source('Vs', 5.0))], reference_node_label='n2')
    passive_network = remove_active_elements(network)

    assert len(passive_network.branches) == 1
    assert passive_network.reference_node_label == network.reference_node_label
...
def test_linear_voltage_sources_are_substituted_by_inner_resisotr() -> None:
    network = Network([Branch('n1', 'n2', elm.voltage_source('Vs', V=5.0, Z=3))], reference_node_label='n2')
    passive_network = remove_active_elements(network)
...
def test_current_sources_are_substituted_by_open_circuit() -> None:
    network = Network([Branch('n1', 'n2', elm.current_source('Is', 5.0))], reference_node_label='n2')
    passive_network = remove_active_elements(network)
...
def test_linear_current_sources_are_substituted_by_open_circuit() -> None:
    network = Network([Branch('n1', 'n2', elm.current_source('Is', I=5.0, Y=3))], reference_node_label='n2')
    passive_network = remove_active_elements(network)
