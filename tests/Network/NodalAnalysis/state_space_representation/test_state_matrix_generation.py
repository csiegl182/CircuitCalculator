...
def test_state_matrix_of_transient_network_1() -> None:
    ...
    A, _, _, _ = state_space_matrices(network, {'C' : C})
    assert_almost_equal(A, np.array([[-(R1 + R2)/(C*(R1*R2 + R1*R3 + R2*R3))]]), decimal=5)
...
def test_state_matrix_of_transient_network_3() -> None:
    ...
    A, _, _, _ = state_space_matrices(network, {'C' : C})
    assert_almost_equal(A, np.array([[-1/(R1+R2)/C]]))
...
def test_state_matrix_of_transient_network_4() -> None:
    ...
    A, _, _, _ = state_space_matrices(network, {'C1' : C1, 'C2' : C2})
    assert_almost_equal(A, np.array([[-1/R1/C1, -1/R1/C1], [-1/R1/C2, -(R1+R2)/C2/R1/R2]]))

def test_state_matrix_of_transient_network_5() -> None:
    V0 = 5
    R1, R2, R3 = 10, 20, 30
    L = 0.1
    network = Network([
    ...
    A, _, _, _ = state_space_matrices(network, l_values={'L' : L})
    assert_almost_equal(A, np.array([[-(R1*R2 + R1*R3 + R2*R3)/(L*(R1 + R2))]]))

def test_state_matrix_of_transient_network_6() -> None:
    R1, R2, R3 = 10, 20, 30
    L = 0.1
    V0 = 5
    network = Network([
...
def test_state_matrix_of_transient_network_7() -> None:
    R1, R2, R3 = 10, 20, 30
    L = 0.1
    V0 = 5
    I0 = 1
...
def test_state_matrix_of_transient_network_8() -> None:
    R1, R2 = 10, 20
    L1, L2 = 0.1, 0.2
    V0 = 5
    network = Network([
    ...
    A, _, _, _ = state_space_matrices(network, l_values={'L1' : L1, 'L2' : L2})
    assert_almost_equal(A, np.array([[-(R1+R2)/L1, R2/L1], [R2/L2, -R2/L2]]))

def test_state_matrix_of_transient_network_9() -> None:
    R = 2
    L = 2e-3
    C = 0.5e-3
    V0 = 5
    ...
    A, _, _, _ = state_space_matrices(network, c_values={'C': C}, l_values={'L' : L})
    assert_almost_equal(A, np.array([[0, 1/C], [-1/L, -R/L]]))