...
def test_feedtrough_matrix_of_transient_network_1() -> None:
    Vs = 1
    R1, R2, R3 = 10, 20, 30
    C = 1e-3
    network = Network([
    ...
    _, _, _, D = state_space_matrices(network, c_values={'C' : C})
    assert_almost_equal(D, np.array([[1], [R2*R3/(R1*R2+R2*R3+R1*R3)], [0], [-(R2+R3)/(R1*R2+R2*R3+R1*R3)]]), decimal=5)
...
def test_feedthrough_matrix_of_transient_network_4() -> None:
    ...
    _, _, _, D = state_space_matrices(network, c_values={'C1' : C1, 'C2' : C2})
    assert_almost_equal(D, np.array([[1], [0], [0], [-1/R1]]))

def test_feedthrough_matrix_of_transient_network_5() -> None:
    V0 = 5
    R1, R2, R3 = 10, 20, 30
    L = 0.1
    network = Network([
    ...
    _, _, _, D = state_space_matrices(network, l_values={'L' : L})
    assert_almost_equal(D, np.array([[1], [R2/(R1+R2)], [R2/(R1+R2)], [0], [-1/(R1+R2)]]))

def test_feedthrough_matrix_of_transient_network_6() -> None:
    R1, R2, R3 = 10, 20, 30
    L = 0.1
    V0 = 5
    network = Network([
    ...
    _, _, _, D = state_space_matrices(network, l_values={'L' : L})
    assert_almost_equal(D, np.array([[1], [R2/(R1+R2)], [0], [0], [-1/(R1+R2)]]))

def test_feedthrough_matrix_of_transient_network_7() -> None:
    R1, R2, R3 = 10, 20, 30
    L = 0.1
    V0 = 5
    I0 = 1
    ...
    _, _, _, D = state_space_matrices(network, l_values={'L' : L})
    assert_almost_equal(D, np.array([[0, 1], [0, R2/(R1+R2)], [R3, 0], [0, 0], [0, -1/(R1+R2)]]))

def test_feedthrough_matrix_of_transient_network_8() -> None:
    R1, R2 = 10, 20
    L1, L2 = 0.1, 0.2
    V0 = 5
    network = Network([
