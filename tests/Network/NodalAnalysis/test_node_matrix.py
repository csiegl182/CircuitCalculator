...
from CircuitCalculator.Network.elements import voltage_source, conductance
import numpy as np
...
def test_node_admittance_matrix_is_correct() -> None:
    ...
    network = Network([
        Branch('1', '2', conductance('Y_12', Y_12)),
        Branch('2', '0', conductance('Y_20', Y_20)),
        Branch('2', '3', conductance('Y_23', Y_23)),
        Branch('3', '4', conductance('Y_34', Y_34)),
        Branch('4', '0', conductance('Y_40', Y_40))
    ])
    Y = node_admittance_matrix(network, NumPyMatrixOperations(), default_label_mappings_factory(network))
...
def test_node_admittance_matrix_sorts_node_indices() -> None:
    ...
    network = Network([
        Branch('1', '0', conductance('Y_10', Y_10)),
        Branch('2', '0', conductance('Y_20', Y_20)),
        Branch('2', '1', conductance('Y_12', Y_12))
    ])
    Y = node_admittance_matrix(network, NumPyMatrixOperations(), default_label_mappings_factory(network))
