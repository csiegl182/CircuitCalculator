...
def test_network_20_with_advanced_nodal_analysis() -> None:
    ...
    solution = numeric_nodal_analysis_bias_point_solution(network)
    np.testing.assert_almost_equal(solution.get_voltage('Vs1'), 1.0, decimal=2)
    np.testing.assert_almost_equal(solution.get_current('Vs1'), -0.5, decimal=2)
    np.testing.assert_almost_equal(solution.get_voltage('R1'), 0.5, decimal=2)
    np.testing.assert_almost_equal(solution.get_current('R1'), 0.5, decimal=2)
    np.testing.assert_almost_equal(solution.get_voltage('R2'), 0.5, decimal=2)
    np.testing.assert_almost_equal(solution.get_current('R2'), 0.5, decimal=2)
...
def test_network_21_with_advanced_nodal_analysis() -> None:
    network = load_network_from_json(str(json_root / 'example_network_21.json'))
    solution = numeric_nodal_analysis_bias_point_solution(network)
    np.testing.assert_almost_equal(solution.get_voltage('Vs'), 1.0, decimal=2)
    np.testing.assert_almost_equal(solution.get_current('Vs'), -1.0, decimal=2)
    np.testing.assert_almost_equal(solution.get_voltage('R1'), 1.0, decimal=2)
    np.testing.assert_almost_equal(solution.get_current('R1'), 1.0, decimal=2)
    with pytest.raises(KeyError): np.isnan(solution.get_voltage('Rx'))
    with pytest.raises(KeyError): np.isnan(solution.get_current('Rx'))
    with pytest.raises(KeyError): np.isnan(solution.get_potential('a'))
    with pytest.raises(KeyError): np.isnan(solution.get_potential('b'))
